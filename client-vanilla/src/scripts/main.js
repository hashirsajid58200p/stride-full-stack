...
async function initApp() {
  try {
    ...
    // DEPLOYMENT VARIANT: (Commented out)
    // const response = await fetch("/api/config");

    // LOCAL MACHINE VARIANT: (Active)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!document.getElementById("global-toast-container")) {
    ...
    loadComponent("../components/notification.html", "notification-placeholder", "initNotifications");
  }
});
...
async function initCurrencySystem() {
  ...
  try {
    ...
    // LOCAL MACHINE VARIANT: (Active - Need the full URL for cross-origin local fetch)
