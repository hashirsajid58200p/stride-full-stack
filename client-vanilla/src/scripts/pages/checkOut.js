...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("currencyUpdated", () => {
    const priceDisplays = document.querySelectorAll(".shipping-price");
    priceDisplays.forEach(el => {
      const rawCost = parseFloat(el.getAttribute("data-cost"));
      if (!isNaN(rawCost)) {
         el.textContent = rawCost === 0 ? "Free" : window.formatPrice(rawCost);
      }
    });
    ...
    form1.addEventListener("submit", (e) => {
      ...
      if (cartItems.length === 0) {
        if (typeof window.showToast === "function") {
          window.showToast("Your cart is empty! Please add items to continue.", "error");
        }
        return;
      }
      ...
      // ==========================================
      // FIX: SAVE ALL FORM DATA TO LOCAL STORAGE
      // ==========================================
      ...
      const apartment = document.getElementById("chk-apartment").value;
      ...
      localStorage.setItem("strideCheckoutData", JSON.stringify(checkoutData));
      // ==========================================
  ...
  // Event listeners for dynamically loaded shipping options are handled within loadShippingOptions
  ...
  async function loadShippingOptions() {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        container.innerHTML = '<p class="text-muted">No shipping options available.</p>';
        return;
      }

      container.innerHTML = data.map((method, index) => {
        const isChecked = index === 0 ? "checked" : "";
        const costVal = parseFloat(method.cost);
        const costDisplay = costVal === 0 ? "Free" : (window.formatPrice ? window.formatPrice(costVal) : `$${costVal.toFixed(2)}`);
        return `
          <label class="shipping-option">
            <div class="shipping-left">
              <input type="radio" name="shipping-rate" value="${costVal}" ${isChecked} />
              <div class="shipping-info">
                <span class="shipping-title">${method.name}</span>
                <span class="shipping-desc">${method.estimated_time}</span>
              </div>
            </div>
            <span class="shipping-price" data-cost="${costVal}">${costDisplay}</span>
          </label>
        `;
      }).join("");

      const shippingRadios = document.querySelectorAll('input[name="shipping-rate"]');
  ...
  if (window.supabase) {
    ...
    window.addEventListener("supabaseInitialized", loadShippingOptions);
  }
